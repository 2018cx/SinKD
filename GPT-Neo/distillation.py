...
def main():
    ...
    #tmodel = GeminiDDP(tmodel, device=get_current_device(), placement_policy=PLACEMENT_POLICY, pin_memory=True)
    ...
    def calculate_loss(model, model2,input_ids, attention_mask, labels, pad_token_id):
        losskl = KL()
        logits_output = []
        logits_output2 = []
        loss_fct = torch.nn.CrossEntropyLoss(ignore_index=-100)
        generated_output = input_ids
        generated_output1 = input_ids
        for i in range(0, input_ids.size(1)):
            ...
            with torch.no_grad():
            # 更新 attention_mask 以匹配新生成的 token
                outputs = model2(input_ids=generated_output1, attention_mask=attention_mask)
                logits = outputs.logits[:, -1, :]
                logits_output2.append(logits)
                next_token = torch.argmax(logits, dim=-1).unsqueeze(1)
                generated_output1 = torch.cat([generated_output1, next_token], dim=1)
                new_attention_mask = torch.ones_like(next_token, dtype=torch.int64)
                attention_mask = torch.cat([attention_mask, new_attention_mask], dim=1)
            
        logits_output = torch.stack(logits_output, dim=1)
        logits_output2 = torch.stack(logits_output2, dim=1)
        loss=0.1 * losskl(logits_output2 , logits_output)
        loss = loss+loss_fct(logits_output.view(-1, logits_output.size(-1)), labels[:,:s].view(-1))
        return loss
    ...
    if args.parallelize:
        ...
        model,tmodel, optimizer, train_dataloader, eval_dataloader = accelerator.prepare(
            model,tmodel, optimizer, train_dataloader, eval_dataloader)
